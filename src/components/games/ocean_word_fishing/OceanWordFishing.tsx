...
type Fish = {
  ...
  y: number; // %
  facing: 1 | -1;
  swimRange: number; // qancha masofaga suzishi (%)
  swimDuration: number;
  swimDelay: number;
  ...
  caught: boolean;
  variant: FishVariant;
  imageUrl: string; // Baliq rasmi URL'i
};
...
const FISH_COLORS = ["#f97316", "#f43f5e", "#facc15", "#14b8a6", "#22d3ee", "#60a5fa", "#34d399"];
...
const getRoundTime = (word: string) => Math.max(35, Math.min(90, 25 + word.length * 7));
...
const buildLettersForRound = (word: string) => {
  ...
  const distractorCount = Math.max(5, Math.min(12, Math.ceil(targetLetters.length * 1.2)));
  const available = ALPHABET.split("").filter((char) => !targetLetters.includes(char));
...
const createFishes = (letters: string[]) => {
  // Har bir baliq uchun qatorlar (5-7 qator)
  const lanes = clamp(Math.ceil(letters.length / 3.5), 5, 7);
  
  // Y pozitsiyalari - ekranning 15% dan 85% gacha
  const topMin = 18;
  const topMax = 82;
  ...
  return letters.map((letter, idx) => {
    ...
    // 70% chapdan o'ngga, 30% o'ngdan chapga suzsin
    const facing = (Math.random() < 0.7 ? 1 : -1) as 1 | -1;
    
    const size = 70 + Math.random() * 40;
    
    // Y pozitsiyasi
    const baseY = topMin + lane * laneStep;
    const yJitter = Math.random() * 12 - 6;
    const y = clamp(baseY + yJitter, topMin, topMax);
    
    // X boshlang'ich pozitsiyasi
    let baseX;
    if (facing === 1) {
      // Chapdan o'ngga suzadi
      baseX = 5 + Math.random() * 10;
    } else {
      // O'ngdan chapga suzadi
      baseX = 75 + Math.random() * 15;
    }
    
    // Suzish oralig'i
    const swimRange = 60 + Math.random() * 20;
    
    // Suzish tezligi
    const swimDuration = 14 + Math.random() * 8;
    
    // Har bir baliq o'ziga xos bo'lsin
    const swimDelay = Math.random() * 3;
    const floatDuration = 2.5 + Math.random() * 2.5;
    
    // Baliq variantini tanlash (0, 1 yoki 2)
...
const FishImage = ({ fish }: { fish: Fish }) => {
  ...
  // Agar rasm yuklanmasa, rangli background ko'rsatamiz
    ...
    return (
      <div
        ...
        style={{
          ...
          transform: fish.facing > 0 ? 'scaleX(1)' : 'scaleX(-1)',
        }}
      >
...
export default function OceanWordFishing() {
  ...
  const foundCount = useMemo(() => words.filter((word) => word.found).length, [words]);
  ...
  const advanceRound = (isFound: boolean) => {
    ...
    if (isFound) {
      setWords((prev) => prev.map((word, wIdx) => (wIdx === idx ? { ...word, found: true } : word)));
    }
  ...
  const addWord = () => {
    ...
    if (words.some((item) => item.word === word)) return setDraftError("Bu so'z allaqachon qo'shilgan.");
  ...
  const handleFishClick = (fish: Fish) => {
    ...
    setFishes((prev) => prev.map((item) => (item.id === fish.id ? { ...item, caught: true } : item)));
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      >
        ...
        <div className="absolute -bottom-16 left-0 right-0 h-44 bg-gradient-to-t from-sky-200/10 to-transparent animate-wave-slow" style={{ animationDelay: "1s" }} />
        <div className="absolute -bottom-12 left-0 right-0 h-36 bg-gradient-to-t from-blue-300/15 to-transparent animate-wave-slower" style={{ animationDelay: "2s" }} />
      ...
      {phase === "finish" && (
        <Confetti mode="boom" particleCount={120} effectCount={1} x={0.5} y={0.35} colors={["#22d3ee", "#3b82f6", "#0ea5e9", "#34d399", "#facc15"]} />
      )}
          ...
          <div className="rounded-3xl border border-cyan-200/35 bg-gradient-to-br from-sky-950/75 to-cyan-950/70 p-7 shadow-2xl backdrop-blur-xl">
            <div className="mb-6 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-cyan-100/80">O'qituvchi so'zlarni kiritsin, bolalar baliq tutib so'z yig'adi.</p>
              </div>
            </div>
            ...
            <div className="mb-7">
              <div className="flex flex-col gap-3 md:flex-row">
                ...
                />
                <button onClick={addWord} className="rounded-xl bg-gradient-to-r from-sky-500 to-cyan-500 px-6 py-3 text-base font-black text-white transition hover:scale-[1.02]">
            ...
            <div className="grid max-h-80 gap-3 overflow-y-auto pr-1">
              {words.map((word, idx) => (
                <div key={word.id} className="flex items-center justify-between rounded-xl border border-cyan-200/25 bg-white/10 p-4">
                  <div className="flex items-center gap-3">
                    <span className="flex h-7 w-7 items-center justify-center rounded-full bg-cyan-500 text-sm font-black text-white">{idx + 1}</span>
                    <div>
                      <p className="text-lg font-black text-white">{word.word}</p>
                      <p className="text-xs text-cyan-100/75">Ball: {word.points}</p>
                    </div>
                  </div>
                  <button onClick={() => removeWord(word.id)} className="text-rose-300 transition hover:text-rose-200">
                    <FaTrash size={16} />
                  </button>
                </div>
              ))}

              {words.length === 0 && <div className="rounded-xl border border-dashed border-cyan-200/30 p-8 text-center text-cyan-100/60">Hozircha so'zlar yo'q</div>}
            </div>

            {words.length > 0 && (
              <div className="mt-7 text-center">
                <button onClick={startGame} className="rounded-2xl bg-gradient-to-r from-sky-500 to-cyan-500 px-10 py-4 text-xl font-black text-white shadow-2xl transition hover:scale-[1.03]">
            ...
            <div className="mx-auto flex max-w-7xl flex-col gap-4 md:flex-row md:items-center md:justify-between">
              ...
              <div className="max-w-3xl flex-1 rounded-xl border border-cyan-200/25 bg-white/10 p-3 backdrop-blur-md">
                  ...
                  {currentWord.letters.map((letter, i) => (
                    <span
                      ...
                      className={`flex h-10 w-10 items-center justify-center rounded-lg border-2 text-lg font-black ${
                        i < selectedLetters.length ? "border-emerald-300 bg-emerald-500 text-white" : "border-cyan-100/35 bg-white/15 text-cyan-100/65"
                      }`}
                    >
                ...
                <div className="h-2 rounded-full bg-white/15">
                  <div className="h-full rounded-full bg-gradient-to-r from-sky-400 to-cyan-400" style={{ width: `${progress}%` }} />
                </div>
              </div>

              <div className="flex gap-2">
                <button onClick={toggleMute} className="flex h-11 w-11 items-center justify-center rounded-xl border border-cyan-200/30 bg-white/15 text-white hover:bg-white/25 transition-all">
                  {isMuted ? <FaVolumeMute /> : <FaVolumeUp />}
                </button>
                <button onClick={resetCurrentRound} className="flex h-11 w-11 items-center justify-center rounded-xl border border-cyan-200/30 bg-white/15 text-white hover:bg-white/25 transition-all">
                  <FaRedo />
                </button>
              </div>
            </div>
                ...
                !fish.caught && (
                  <div
                    ...
                    style={{
                      ...
                      animation: `fish-swim-${fish.facing === 1 ? 'right' : 'left'} ${fish.swimDuration}s linear infinite ${fish.swimDelay}s`,
                    }}
                  >
                    {/* Vertikal tebranish */}
                    <div
                    ...
                    >
                      ...
                      >
                        <span className="absolute -inset-5" /> {/* Kattaroq bosish maydoni */}
                        <FishImage fish={fish} />
                      </button>
                    </div>
                  </div>
            ...
            <div className="rounded-2xl border border-cyan-100/30 bg-sky-950/65 p-4 backdrop-blur-lg">
              ...
              <div className="flex min-h-11 gap-2">
                {selectedLetters.length > 0 ? (
                  ...
                  <span className="px-3 py-2 text-sm text-cyan-100/60">Baliqlarni bosib so'zni yig'ing</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-cyan-100/35 bg-gradient-to-br from-sky-950/80 via-cyan-950/75 to-teal-950/75 p-8 text-center shadow-2xl backdrop-blur-xl">
            ...
            <div className="mx-auto mb-6 grid max-w-md grid-cols-2 gap-3">
              <div className="rounded-xl border border-cyan-100/30 bg-white/10 p-4">
                ...
                <p className="text-2xl font-black text-white">{foundCount}/{words.length}</p>
              </div>
              <div className="rounded-xl border border-cyan-100/30 bg-white/10 p-4">
