...
function TodoList() {
  ...
  const navigate = useNavigate();
   ...
   const { todos, addTodo, updateTodo, deleteTodo, toggleTodoCompleted, getTodosByUserId } = useTodos(todoId);
  ...
  const handleDeleteTodo = (id: number) => {
    ...
    useEffect(() => {
        if (todoId) {
            getTodosByUserId(todoId);
        }
        }, [todoId]);

    const filteredTodos = todos.filter((t: Todo) => t.user_id === todoId);
